...
import { BarChart3, CalendarClock, CheckCircle2, ClipboardList, FileText, Sparkles, TrendingUp, Users } from 'lucide-react';
...
type DashboardTask = {
  ...
  meta: string;
  badge: string;
  tone: 'brand' | 'success' | 'warning' | 'danger' | 'neutral' | 'muted' | 'violet' | 'sky';
  sortDate: string;
  urgency: number;
};
...
function isDemoLead(lead: Lead) {
  return getContext(lead).demoData === true;
}
...
export default function Dashboard() {
    ...
    (async () => {
      try {
        ...
        const [leadsRes, bucketsRes] = await Promise.all([
          fetch('/api/leads?limit=200&exclude_demo=true', { cache: 'no-store' }),
          fetch('/api/buckets', { cache: 'no-store' }),
        ]);
        const [leadsJson, bucketsJson] = await Promise.all([
          leadsRes.json(),
          bucketsRes.json(),
        ]);

        const firstError = leadsJson.error || bucketsJson.error;
        if (firstError) throw new Error(firstError);

        if (!cancelled) {
          setLeads((leadsJson.leads || []).filter((lead: Lead) => !isDemoLead(lead)));
  ...
  const stats = useMemo(() => {
    ...
    const scoredLeads = leads.filter((lead) => typeof lead.ai_score === 'number');
    const averageScore = scoredLeads.length
      ? Math.round(scoredLeads.reduce((sum, lead) => sum + Number(lead.ai_score || 0), 0) / scoredLeads.length)
      : 0;
  ...
  const tasksInProgress = useMemo<DashboardTask[]>(() => {
    const tasks: DashboardTask[] = [];

    for (const lead of leads) {
      const href = `/crm/leads/${lead.id}`;
      const leadName = lead.customer_name || lead.issue || 'CRM lead';
      const sortDate = lead.next_follow_up_at || lead.created_at;

      if (isDue(lead.next_follow_up_at)) {
        tasks.push({
          id: `${lead.id}-follow-up`,
          title: `Follow up with ${leadName}`,
          href,
          meta: lead.issue,
          badge: formatDateLabel(lead.next_follow_up_at),
          tone: 'danger',
          sortDate,
          urgency: 1,
        });
      ...
      if (lead.status === 'proposal_sent') {
        tasks.push({
          id: `${lead.id}-proposal`,
          title: `Confirm proposal decision with ${leadName}`,
          href,
          meta: lead.issue,
          badge: formatCurrency(lead.estimated_value_cents || 0, { cents: true }),
          tone: 'warning',
          sortDate,
          urgency: 2,
        });
      ...
      if (lead.status === 'planning_call_scheduled') {
        tasks.push({
          id: `${lead.id}-planning-call`,
          title: `Prepare planning call for ${leadName}`,
          href,
          meta: lead.issue,
          badge: 'Planning call',
          tone: 'brand',
    ...
    return tasks
      .sort((a, b) => a.urgency - b.urgency || String(a.sortDate).localeCompare(String(b.sortDate)))
      .slice(0, 6);
  }, [leads]);
      ...
      <main className="min-h-0 flex-1 overflow-auto px-4 py-6 sm:px-6 xl:px-8">
        <div className="mx-auto flex w-full max-w-[1800px] flex-col gap-6">
          <AppPageHeader
            ...
            description={`${awpBusinessProfile.shortName} dashboard for live CRM leads, pipeline movement, open follow-ups, and estimated opportunity value.`}
            actions={
              <>
          ...
          <KpiStrip className="xl:grid-cols-4 2xl:grid-cols-8">
            ...
            <StatCard label="Average Lead Score" value={loading ? '...' : stats.averageScore || '-'} meta="Qualified CRM leads" tone="sky" icon={CheckCircle2} />
          ...
          <div className="grid gap-6 xl:grid-cols-[minmax(0,1.35fr)_420px]">
            <div className="space-y-6">
              <ConsolePanel title="Recent Leads" description="Newest live cabin buyer and partner opportunities from the CRM.">
                <DataTable
                  columns={[
                    { key: 'name', label: 'Lead' },
              ...
              <ConsolePanel title="Pipeline Overview" description="A compact view of the Cabin Lead Pipeline stages.">
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {pipelineCounts.length === 0 ? (
                    <p className="rounded-lg border border-dashed border-[var(--ops-border-strong)] bg-[var(--ops-surface-subtle)] px-4 py-6 text-center text-sm text-[var(--ops-muted)] md:col-span-2 xl:col-span-3">
            ...
            <div className="space-y-6 xl:sticky xl:top-6">
              ...
              <ConsolePanel title="Tasks in Progress" description="Lead work that needs action, built from live CRM stages and follow-up dates." icon={ClipboardList}>
                <div className="space-y-3">
                  {tasksInProgress.length === 0 ? (
                    ...
                    tasksInProgress.map((task) => (
                      <Link key={task.id} href={task.href} className="block rounded-[20px] border border-[var(--ops-border)] bg-[var(--ops-surface-strong)] px-4 py-3 hover:bg-[var(--ops-surface-subtle)]">
                        <div className="flex items-start justify-between gap-3">
                          <p className="text-sm font-semibold text-[var(--ops-text)]">{task.title}</p>
                          <StatusBadge tone={task.tone}>{task.badge}</StatusBadge>
                        </div>
                        <p className="mt-1 text-xs text-[var(--ops-muted)]">{task.meta}</p>
                      </Link>
                    ))
                  )}
                </div>
